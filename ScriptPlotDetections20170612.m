...
if ispc,
  ...
  preddatadir = '/nrs/turaga/bergera/division_detection/prediction_outbox/mk5_large_small_batch_balanced_good_reweight/dense';
...
outdir = '/nrs/branson/MouseLineaging/mk5_large_small_batch_balanced_good_reweight_sparse';
if ~exist(outdir,'dir'),
  mkdir(outdir);
end
...
for i = 1:numel(predtimestamps),
  ...
  outmatfile = fullfile(outdir,[namecurr,'.mat']);
  if exist(outmatfile,'file'),
    continue;
  end
  scriptfile = fullfile(outdir,['SaveSparse_',namecurr,'.sh']);
  logfile = fullfile(outdir,['SaveSparse_',namecurr,'.log']);
...
for i = 1:numel(predtimestamps),
  ...
  preddatafiles{i} = fullfile(outdir,[namecurr,'.mat']);
...
for i = 386:numel(predtimestamps),
...
%     clf;
%     hax = gca;
%     him = imagesc(predmaxv);
%     axis image; truesize; hold on;
%   else
%     set(him,'CData',predmaxv);
%     set(hax,'CLim',[0,max(eps,max(predmaxv(:)))]);
%   end
%   
%   labelis = find(labellocs(:,4) == t);
%   if ~ishandle(hlabel),
%     if ~isempty(labelis),
%       hlabel = plot(labellocs(labelis,2),labellocs(labelis,1),'o','Color',[1,.6,0]);
%     end
%   else
%     set(hlabel,'XData',labellocs(labelis,2),'YData',labellocs(labelis,1));
%   end
%   
%   drawnow;
...
for d = 1:3,
  minv(:,d) = accumarray(allpredidx(:,4),allpredidx(:,d),[T,1],@min,single(inf));
  maxv(:,d) = accumarray(allpredidx(:,4),allpredidx(:,d),[T,1],@max,single(-inf));
end
...
for ii = 1:numel(idxrerun),
  ...
  scriptfile = fullfile(outdir,['SaveSparseRerun_',namecurr,'.sh']);
  logfile = fullfile(outdir,['SaveSparseRerun_',namecurr,'.log']);
...
outdir = '/nrs/branson/MouseLineaging';
inmatfilestr = '/nrs/branson/MouseLineaging/imregionalmaxdata';
nchunksperjob = 100;

inmatfilestr_conncomp = '/nrs/branson/MouseLineaging/conncompdata';
...
for spliti = 1:nsplit,
  ...
  imregionalmax_list2(allpredidx(j0:j1,:),sz,'filsig',filsig,'filrad',filrad,'chunksize',chunksize,'chunkstep',chunkstep,...
    'startrunoncluster',true,'outdir',outdir,'inmatfile',inmatfile,'ncores',ncores,'nchunksperjob',nchunksperjob,...
...
for spliti = 1:nsplit,
  ...
  bwconncomp_list2(nonmaxpredidx0{spliti},sz,'chunksize',chunksize_conncomp,'chunkstep',chunkstep_conncomp,...
    ...
    'startrunoncluster',true,'outdir',outdir,'inmatfile',inmatfile,'ncores',ncores,'nchunksperjob',nchunksperjob_conncomp,...
    'scriptfilestr',scriptfilestr,'outmatfilestr',outmatfilestr,'logfilestr',logfilestr);
...
save detections5.mat nonmaxpredidx0 nonmaxpredidx nonmaxpredidx_combined scores peaks
